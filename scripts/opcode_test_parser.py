...
TEST_FUNCTION_OPCODES = {
    ...
    "test_worst_zero_param": [
        "ADDRESS", "ORIGIN", "CALLER", "CODESIZE", "GASPRICE", "COINBASE",
    ...
    "test_worst_address_state_warm": [
        "BALANCE", "EXTCODESIZE", "EXTCODEHASH", "CALL", "CALLCODE", "DELEGATECALL", "STATICCALL",
    ],
    ...
    "test_worst_bytecode_single_opcode": [
        "EXTCODESIZE", "EXTCODEHASH", "EXTCODECOPY", "CALL", "CALLCODE", "DELEGATECALL", "STATICCALL",
    ],
...
class OpcodeTestParser:
    ...
    def _determine_opcodes(
        ...
        # 2. Check raw parameters for specific opcodes (like "log0", "log1", etc.)
        # This should take priority over function mappings
        # Split parameters and check last segment for opcode names
        param_parts = raw_params.split("-")
        for part in param_parts:
            part_upper = part.upper()
            # Check if this part is a known opcode (exact match)
            if part_upper in ["LOG0", "LOG1", "LOG2", "LOG3", "LOG4", 
                             "DUP1", "DUP2", "DUP3", "DUP4", "DUP5", "DUP6", "DUP7", "DUP8",
                             "DUP9", "DUP10", "DUP11", "DUP12", "DUP13", "DUP14", "DUP15", "DUP16",
                             "SWAP1", "SWAP2", "SWAP3", "SWAP4", "SWAP5", "SWAP6", "SWAP7", "SWAP8",
                             "SWAP9", "SWAP10", "SWAP11", "SWAP12", "SWAP13", "SWAP14", "SWAP15", "SWAP16",
                ...
                opcodes.append(part_upper)
                return self._deduplicate_opcodes(opcodes)
        
        # 3. Check for precompile address
        ...
        # 4. Check for precompile name in parameters
        ...
        # 5. Try to extract from raw parameters using patterns
        ...
        # 6. Look up function name in mapping
        ...
        # 7. Try to infer from function name patterns
        opcodes.extend(self._infer_from_function_name(function_name))
        
        if opcodes:
            return self._deduplicate_opcodes(opcodes)
        
        # Note: Step 2 already checked raw parameters for opcodes, so we skip that here
    ...
    def _infer_from_function_name(self, function_name: str) -> List[str]:
        ...
        if function_name.startswith("test_worst_"):
            ...
            known_opcodes = [
                "ADD", "MUL", "SUB", "DIV", "SDIV", "MOD", "SMOD", "EXP", "ADDMOD", "MULMOD",
                "SIGNEXTEND", "LT", "GT", "SLT", "SGT", "EQ", "ISZERO", "AND", "OR", "XOR",
                "NOT", "BYTE", "SHL", "SHR", "SAR", "SHA3", "KECCAK256", "ADDRESS", "BALANCE",
                "ORIGIN", "CALLER", "CALLVALUE", "CALLDATALOAD", "CALLDATASIZE", "CALLDATACOPY",
                "CODESIZE", "CODECOPY", "GASPRICE", "EXTCODESIZE", "EXTCODECOPY", "EXTCODEHASH",
                "RETURNDATASIZE", "RETURNDATACOPY", "BLOCKHASH", "COINBASE", "TIMESTAMP", "NUMBER",
                "PREVRANDAO", "GASLIMIT", "CHAINID", "SELFBALANCE", "BASEFEE", "POP", "MLOAD",
                "MSTORE", "MSTORE8", "SLOAD", "SSTORE", "JUMP", "JUMPI", "PC", "MSIZE", "GAS",
                "JUMPDEST", "TLOAD", "TSTORE", "MCOPY", "LOG0", "LOG1", "LOG2", "LOG3", "LOG4",
                "CREATE", "CREATE2", "CALL", "CALLCODE", "RETURN", "DELEGATECALL", "STATICCALL",
                "REVERT", "SELFDESTRUCT", "BLOBHASH", "BLOBBASEFEE", "CLZ",
            ]
    ...
    def _find_opcodes_in_text(self, text: str) -> List[str]:
        ...
        all_opcodes = [
            "SIGNEXTEND", "CALLDATACOPY", "CALLDATALOAD", "CALLDATASIZE", "RETURNDATACOPY",
            "RETURNDATASIZE", "EXTCODECOPY", "EXTCODESIZE", "EXTCODEHASH", "DELEGATECALL",
            "STATICCALL", "SELFDESTRUCT", "BLOBBASEFEE", "SELFBALANCE", "PREVRANDAO",
            "KECCAK256", "ADDMOD", "MULMOD", "ISZERO", "CODECOPY", "CODESIZE", "BALANCE",
            "CALLCODE", "JUMPDEST", "BLOCKHASH", "BLOBHASH", "COINBASE", "TIMESTAMP",
            "GASLIMIT", "GASPRICE", "CHAINID", "BASEFEE", "MSTORE8", "CREATE2", "CALLVALUE",
            "ADD", "MUL", "SUB", "DIV", "SDIV", "MOD", "SMOD", "EXP", "LT", "GT", "SLT",
            "SGT", "EQ", "AND", "OR", "XOR", "NOT", "BYTE", "SHL", "SHR", "SAR", "SHA3",
            "LOG0", "LOG1", "LOG2", "LOG3", "LOG4", "SLOAD", "SSTORE", "TLOAD", "TSTORE",
            "MLOAD", "MSTORE", "MCOPY", "MSIZE", "CALL", "CREATE", "JUMP", "JUMPI",
            "RETURN", "REVERT", "PUSH0", "PUSH1", "DUP1", "SWAP1", "ADDRESS", "ORIGIN",
            "CALLER", "NUMBER", "GAS", "CLZ",
        ]
