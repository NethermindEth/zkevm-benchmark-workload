...
use reth_stateless::{stateless_execution_with_trie, Genesis, StatelessInput};
...
pub fn main() {
    ...
    println!("start stateless execution");
    let start = env::cycle_count();
    let res = stateless_execution_with_trie::<SparseState, _, _>(
        input.block,
        input.witness,
        chain_spec,
        evm_config,
    ...
    let end = env::cycle_count();
    eprintln!("stateless execution (cycle tracker): {}", end - start);
