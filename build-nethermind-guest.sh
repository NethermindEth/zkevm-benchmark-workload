...
NETHERMIND_REF="${NETHERMIND_REF:-feature/benchmark}"
...
Options:
  ...
  --ref GIT_REF      Branch / tag / commit to check out (default: feature/benchmark).
...
# Nethermind.Analyzers is hooked into every project via Directory.Build.props as
# an Analyzer ProjectReference. The built analyzer DLL targets Roslyn 5.3 while
# dotnet 10.0.x SDK ships Roslyn 5.0, so every consuming project trips CS9057
# when the compiler tries to load it. -p:RunAnalyzers=false only stops execution
# (the DLL is still loaded) and doesn't help — strip the ProjectReference block.
DBP="${NETHERMIND_SRC}/src/Nethermind/Directory.Build.props"
if grep -q 'OutputItemType="Analyzer"' "${DBP}"; then
    log "Stripping Nethermind.Analyzers ProjectReference from ${DBP}"
    sed -i.bak -e '/<!-- Exclude the analyzer/,/<\/ItemGroup>/d' "${DBP}"
fi
