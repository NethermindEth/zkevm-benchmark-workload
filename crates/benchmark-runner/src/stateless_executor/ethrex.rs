...
use ethrex_executor_guest::guest::{EthrexStatelessExecutorGuest, EthrexStatelessExecutorInput};
...
/// Prepares the inputs for the Ethrex stateless executor guest program.
...
pub fn stateless_executor_inputs_from_fixtures(
    ...
    fixtures
        ...
        .map(|bw| {
            ...
            Ok(GenericGuestFixture::<EthrexStatelessExecutorGuest, _> {
                name: bw.name.clone(),
                input,
                metadata,
...
fn get_input_full_execution(
...
) -> anyhow::Result<EthrexStatelessExecutorInput> {
    ...
    Ok(EthrexStatelessExecutorInput(ethrex_program_input))
}
