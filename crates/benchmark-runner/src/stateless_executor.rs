...
pub enum ExecutionClient {
    /// Reth stateless block execution guest program.
    Reth,
    /// Ethrex stateless block execution guest program.
    Ethrex,
}
...
/// Prepares the inputs for the stateless executor guest program based on the mode.
...
pub fn stateless_executor_inputs_from(
    ...
    match el {
        ExecutionClient::Reth => reth::stateless_executor_inputs_from_fixtures(fixtures),
        ExecutionClient::Ethrex => ethrex::stateless_executor_inputs_from_fixtures(fixtures),
    }
}
...
pub fn read_benchmark_fixtures_folder(path: &Path) -> Result<Vec<StatelessExecutionFixture>> {
    WalkDir::new(path)
        .min_depth(1)
        .into_iter()
        ...
        .map(|entry| {
            if entry.file_type().is_file() {
                ...
                let bw: StatelessExecutionFixture =
